...
typedef void(*raw_audio_callback)(unsigned char* raw_data, unsigned long len, long user_data);


class capture_audio : public sound_handler
{
public:
	void start(int samples_per_sec, int channel, int bits_per_sample, raw_audio_callback cb, long user_data)
	{
		this->cb_ = cb;
